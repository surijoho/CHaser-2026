import socket
import ipaddress
import os

name_in = "数理情太郎"	#自分の名前を入れよう     ※ この記号→" は消さないでね
svr_in = "192.168.128.103"	#試合の時はここも変えるよ 127.0.0.1







# ---------------------- ここから下は変更しません ----------------------

class Client:
    def __init__(self):

        self.port = "2009"
        self.name = name_in
        self.svr = svr_in



        if not self.__ip_judge(self.svr):
            os._exit(1)

        self.client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.client.connect((self.svr, int(self.port)))

        print("port:", self.port)
        print("name:", self.name)
        print("svr:", self.svr)

        print("サーバーの「ゲーム開始」を押してください。")

        self.__str_send(self.name + "\r\n")

    def __ip_judge(self, host):
        try:
            ipaddress.ip_address(host)
        except Exception as e:
            print("IPアドレスの形式に誤りがあります : {0}".format(e))
            return False
        else:
            return True

    def __str_send(self, send_str):
        try:
            self.client.sendall(send_str.encode("utf-8"))
        except:
            print("send error:{0}\0".format(send_str))

    def __order(self, order_str, gr_flag = False):
        try:
            if gr_flag:
                responce = self.client.recv(4096)

                if(b'@' in responce):
                    pass # Connection completed.
                else:
                    print("Connection failed.")

            self.__str_send(order_str + "\r\n")

            responce = self.client.recv(4096)[0:11].decode("utf-8")

            if not gr_flag:
                self.__str_send("#\r\n")

            if responce[0] == '1':
                return [int(x) for x in responce[1:10]]
            elif responce[0] == '0':
                raise OSError("Game Set!")
                a = input("Enterを押してください。")

            else:
                print("responce[0] = {0} : Response error.".format(responce[0]))
                raise OSError("Responce Error")

        except OSError as e:
            print(e)
            self.client.close()
            os._exit(0)

    def gr(self):
        return self.__order("gr", True)

    def wr(self):
        return self.__order("wr")

    def wu(self):
        return self.__order("wu")

    def wl(self):
        return self.__order("wl")

    def wd(self):
        return self.__order("wd")

    def lr(self):
        return self.__order("lr")

    def lu(self):
        return self.__order("lu")

    def ll(self):
        return self.__order("ll")

    def ld(self):
        return self.__order("ld")

    def sr(self):
        return self.__order("sr")

    def su(self):
        return self.__order("su")

    def sl(self):
        return self.__order("sl")

    def sd(self):
        return self.__order("sd")

    def pr(self):
        return self.__order("pr")

    def pu(self):
        return self.__order("pu")

    def pl(self):
        return self.__order("pl")

    def pd(self):
        return self.__order("pd")



class Client2:
    def __init__(self):

        self.port = "2010"
        self.name = name_in
        self.svr = svr_in



        if not self.__ip_judge(self.svr):
            os._exit(1)

        self.client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.client.connect((self.svr, int(self.port)))

        print("port:", self.port)
        print("name:", self.name)
        print("svr:", self.svr)

        print("サーバーの「ゲーム開始」を押してください。")

        self.__str_send(self.name + "\r\n")

    def __ip_judge(self, host):
        try:
            ipaddress.ip_address(host)
        except Exception as e:
            print("IPアドレスの形式に誤りがあります : {0}".format(e))
            return False
        else:
            return True

    def __str_send(self, send_str):
        try:
            self.client.sendall(send_str.encode("utf-8"))
        except:
            print("send error:{0}\0".format(send_str))

    def __order(self, order_str, gr_flag = False):
        try:
            if gr_flag:
                responce = self.client.recv(4096)

                if(b'@' in responce):
                    pass # Connection completed.
                else:
                    print("Connection failed.")

            self.__str_send(order_str + "\r\n")

            responce = self.client.recv(4096)[0:11].decode("utf-8")

            if not gr_flag:
                self.__str_send("#\r\n")

            if responce[0] == '1':
                return [int(x) for x in responce[1:10]]
            elif responce[0] == '0':
                raise OSError("Game Set!")
                a = input("Enterを押してください。")

            else:
                print("responce[0] = {0} : Response error.".format(responce[0]))
                raise OSError("Responce Error")

        except OSError as e:
            print(e)
            self.client.close()
            os._exit(0)

    def gr(self):
        return self.__order("gr", True)

    def wr(self):
        return self.__order("wr")

    def wu(self):
        return self.__order("wu")

    def wl(self):
        return self.__order("wl")

    def wd(self):
        return self.__order("wd")

    def lr(self):
        return self.__order("lr")

    def lu(self):
        return self.__order("lu")

    def ll(self):
        return self.__order("ll")

    def ld(self):
        return self.__order("ld")

    def sr(self):
        return self.__order("sr")

    def su(self):
        return self.__order("su")

    def sl(self):
        return self.__order("sl")

    def sd(self):
        return self.__order("sd")

    def pr(self):
        return self.__order("pr")

    def pu(self):
        return self.__order("pu")

    def pl(self):
        return self.__order("pl")

    def pd(self):
        return self.__order("pd")